...
- `skills/*`: Agent skills (`stash-cli`, `stash-encryption`, `stash-indexing`, `stash-sql`, `stash-edge`, `stash-drizzle`, `stash-dynamodb`, `stash-supabase`, `stash-prisma-next`, `stash-supply-chain-security`)
...
| The EQL operator/domain surface (`eql_v3.query_*` casts, predicate forms) | `skills/stash-sql` |
