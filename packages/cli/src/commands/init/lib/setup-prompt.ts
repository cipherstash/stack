...
const SKILL_PURPOSES: Record<string, string> = {
  ...
  'stash-managed-platforms':
    'managed AI app platforms (Lovable, v0, Bolt, Replit): the WASM entry, headless `auth login --json`, minting `CS_*` with `stash env`, installing EQL as a non-`postgres` role, and which predicates survive PostgREST',
  'stash-deployment':
