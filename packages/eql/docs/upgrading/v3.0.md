...
| JSON containment (`public.eql_v3_json_search` `@>` / `<@`, `eql_v3.jsonb_document_contains`) | **Unchanged.** Genuine containment keeps the containment vocabulary — see U-008. (The implementation was called `eql_v3.ste_vec_contains` until 3.0.5 renamed it; the operators never changed. See [Upgrading to EQL 3.0.5](./v3.0.5.md).) |
...
`eql_v3.jsonb_document_contains` — named `eql_v3.ste_vec_contains` before
3.0.5), which is unaffected and keeps `@>` / `<@`. This is the
...
- Range queries, document-to-document containment (`@>` / `<@`,
  `eql_v3.jsonb_document_contains`, called `eql_v3.ste_vec_contains` before
  3.0.5), field/array access (`->`, `->>`), ordered
