...
An ambient `DATABASE_URL` no longer overrules a declaration: when `schemas` are passed without an explicit `databaseUrl`, the environment variable is ignored and a warning says the declaration is unverified. Previously a stray variable silently exited declared mode — introspecting a database the caller never named on Node, and on the edge entry throwing "drop databaseUrl" about an option never passed.
