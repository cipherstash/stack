...
describe('codec.encode(envelope, ctx)', () => {
  it('extracts the ciphertext from the envelope handle', async () => {
    ...
    expect(wire).toBe(
      `("${JSON.stringify(ciphertextPayload).replaceAll('"', '""')}")`,
    )
  })

  it('returns the envelope unchanged when ciphertext is missing AND the bulk-encrypt middleware is registered for the sdk', async () => {
    // Happy-path encode lifecycle: in the SQL runtime,
    // `lower`/`encodeParams` runs before the `beforeExecute`
    // middleware chain. Throwing here would block the bulk-encrypt
    // middleware from ever running. The codec's contract is therefore
    // "return *something* (the envelope itself is sufficient — it
    // will be replaced via `params.replaceValues(...)`) when no
    // ciphertext is set yet, **provided** the middleware has been
    // wired up against the same sdk."
    const sdk = emptySdk()
    bulkEncryptMiddleware(sdk) // marks `sdk` as registered
    const codec = createCipherstashStringCodec(sdk)
    const envelope = EncryptedString.from('alice@example.com')
    const result = await codec.encode(envelope, ctxWithoutColumn)
    expect(result).toBe(envelope)
  })

  it('throws a clear RUNTIME.ENCODE_FAILED envelope when the bulk-encrypt middleware was never constructed against the sdk', async () => {
    // Misconfig diagnostic: when the user forgets to construct
    // `bulkEncryptMiddleware(sdk)`, the two-pass write path can never
    // complete — there's no middleware to fill in the ciphertext. The
    // codec surfaces the error at the codec boundary with a copy-
    // pasteable wiring snippet, rather than letting the un-encrypted
    // envelope reach the pg driver and produce an opaque serialise
    // error.
    const sdk = emptySdk() // never passed to `bulkEncryptMiddleware`
    const codec = createCipherstashStringCodec(sdk)
    const envelope = EncryptedString.from('alice@example.com')
...
describe('createParameterizedCodecDescriptors', () => {
  ...
  it('renderOutputType returns the per-codec envelope class name', () => {
    ...
    expect(
      doubleDescriptor?.renderOutputType?.({
        equality: true,
        orderAndRange: true,
      }),
    ).toBe('EncryptedDouble')
    expect(
      bigIntDescriptor?.renderOutputType?.({
        equality: true,
        orderAndRange: true,
      }),
    ).toBe('EncryptedBigInt')
    expect(
      dateDescriptor?.renderOutputType?.({
        equality: true,
        orderAndRange: true,
      }),
    ).toBe('EncryptedDate')
    expect(booleanDescriptor?.renderOutputType?.({ equality: true })).toBe(
      'EncryptedBoolean',
    )
    expect(jsonDescriptor?.renderOutputType?.({ searchableJson: true })).toBe(
      'EncryptedJson',
    )
  })
  ...
  it('numeric paramsSchemas accept { equality, orderAndRange } booleans via Standard Schema', () => {
    for (const schema of [
      ...
      if (ok.issues)
        throw new Error(
          `expected success, got issues: ${JSON.stringify(ok.issues)}`,
        )
...
describe('createCipherstashDateCodec — registration shape + round-trip', () => {
  ...
  it('encode → decode round-trip preserves the ciphertext', async () => {
    ...
    const decoded = await codec.decode(
      wire as string,
      ctxWithColumn('event', 'occurred_on'),
    )
...
describe('paramsSchemas for date / boolean / json', () => {
  it('encryptedDateParamsSchema accepts { equality, orderAndRange } booleans', () => {
    ...
    if (ok instanceof Promise)
      throw new Error('expected synchronous validation')
    if (ok.issues)
      throw new Error(`expected success, got: ${JSON.stringify(ok.issues)}`)
    expect(ok.value).toEqual({ equality: true, orderAndRange: false })
  })

  it('encryptedBooleanParamsSchema accepts { equality } and rejects extras of wrong type', () => {
    const ok = encryptedBooleanParamsSchema['~standard'].validate({
      equality: true,
    })
    if (ok instanceof Promise)
      throw new Error('expected synchronous validation')
    ...
    const bad = encryptedBooleanParamsSchema['~standard'].validate({
      equality: 'yes',
    })
    if (bad instanceof Promise)
      throw new Error('expected synchronous validation')
  ...
  it('encryptedJsonParamsSchema accepts { searchableJson } booleans', () => {
    const ok = encryptedJsonParamsSchema['~standard'].validate({
