...
- `json.e2e.test.ts` — `eql_v3_json` round-trip + `eqlJsonContains` (encrypted jsonb `@>`): top-level, multi-key, array-element, and nested containment, negative cases, and the loud rejection of the match-everything `{}` needle.
...
- **JSON predicates work.** v2's `cipherstashJsonbPathExists` needed client-side STE-VEC selector hashing and was skipped. v3's containment model (`eqlJsonContains` → `@>` with an `eql_v3.query_jsonb` term) runs end-to-end — no skips in `json.e2e.test.ts`.
