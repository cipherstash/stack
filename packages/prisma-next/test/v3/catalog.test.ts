...
describe('v3 catalog derivation', () => {
  it('derives a concrete codec id per domain from the registry key verbatim', () => {
    expect(toV3CodecId('eql_v3_text_search')).toBe(
      'cipherstash/eql-v3/eql_v3_text_search@1',
    )
    expect(toV3CodecId('eql_v3_boolean')).toBe(
  ...
  it('records the public.* native type (never eql_v3.*) for text_search with full caps', () => {
    ...
    expect(Object.keys(meta?.indexes ?? {}).sort()).toEqual([
      'match',
      'ope',
      'unique',
    ])
  ...
  it('exposes bigint as a first-class family with bigint castAs', () => {
    expect(
      V3_DOMAIN_META_BY_CODEC_ID.get('cipherstash/eql-v3/eql_v3_bigint@1')
        ?.castAs,
    ).toBe('bigint')
    expect(
      V3_DOMAIN_META_BY_CODEC_ID.get('cipherstash/eql-v3/eql_v3_bigint_ord@1')
        ?.nativeType,
    ).toBe('public.eql_v3_bigint_ord')
  })

  it('derives timestamp castAs distinctly from date', () => {
    expect(
      V3_DOMAIN_META_BY_CODEC_ID.get('cipherstash/eql-v3/eql_v3_timestamp@1')
        ?.castAs,
    ).toBe('timestamp')
    expect(
      V3_DOMAIN_META_BY_CODEC_ID.get('cipherstash/eql-v3/eql_v3_date@1')
        ?.castAs,
    ).toBe('date')
  })
  ...
  it('every codec id is v3-namespaced and unique', () => {
    ...
    for (const id of V3_CODEC_IDS)
      expect(id.startsWith('cipherstash/eql-v3/')).toBe(true)
  })
})
