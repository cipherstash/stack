...
describe('per-command --help', () => {
  it('renders a group listing for `eql --help`', async () => {
    ...
    expect(r.output).toContain('eql verify')
  ...
  it('renders full command help for `eql verify --help`', async () => {
    const r = await run(['eql', 'verify', '--help'], {
      env: { npm_config_user_agent: '' },
    })
    expect(r.exitCode).toBe(0)
    expect(r.output).toContain('Usage: npx stash eql verify [options]')
