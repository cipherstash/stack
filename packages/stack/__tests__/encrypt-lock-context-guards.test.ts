...
describe('encrypt with lock context rejects non-finite numbers', () => {
  it('rejects NaN and never reaches the FFI', async () => {
    const result = await client
      .encrypt(Number.NaN, { column: users.score, table: users })
  ...
  it('rejects +Infinity and never reaches the FFI', async () => {
    const result = await client
      .encrypt(Number.POSITIVE_INFINITY, { column: users.score, table: users })
      .withLockContext(new LockContext())
  ...
  it('rejects -Infinity and never reaches the FFI', async () => {
    const result = await client
      .encrypt(Number.NEGATIVE_INFINITY, { column: users.score, table: users })
      .withLockContext(new LockContext())
  ...
  it('accepts a finite number and forwards it to the FFI', async () => {
    ...
    const result = await client
      .encrypt(42, { column: users.score, table: users })
      .withLockContext(new LockContext())
