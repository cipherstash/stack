...
describe('bulkEncryptMiddleware', () => {
  ...
  describe('ciphertext is stamped onto each envelope handle', () => {
    it('populates handle.ciphertext with the SDK-returned wire value', async () => {
      ...
      expect(envelope.expose().ciphertext).toBe(
        'cipher:user.email:alice@example.com',
      )
    })
  })

  describe('param slot carries the encoded wire-format string post-middleware', () => {
    it('replaces the envelope with the eql_v2_encrypted composite-text literal', async () => {
      // The pg driver only knows how to serialise primitives /
      // arrays / Buffers — passing the `EncryptedEnvelopeBase`
      // instance through would fail at the driver boundary with a
      // confusing serialize error. The middleware therefore writes
      // the wire-format string (the `("...")` composite-text
      // literal) into the param slot via `params.replaceValues`,
      // and the runtime's `currentParams()` view reflects that
      ...
      expect(finalParams.length).toBe(1)
      const onlyParam = finalParams[0]
      expect(typeof onlyParam).toBe('string')
      // Composite-text literal: `("` + escaped JSON of the ciphertext
      // + `")`. Double-quotes inside the JSON are doubled.
      const expectedPayload = JSON.stringify(
        'cipher:user.email:alice@example.com',
      ).replaceAll('"', '""')
      expect(onlyParam).toBe(`("${expectedPayload}")`)
    })
  })
  ...
  describe('routing key is derived from envelope handle (table, column)', () => {
    ...
    it('rejects re-binding a pre-stamped envelope to a different routing target', async () => {
      ...
      ).rejects.toThrow(/routing-key table conflict/)
  ...
  describe('matches every cipherstash codec id', () => {
    ...
    function buildHeterogeneousInsertPlan(
      ...
      for (const col of columns) {
        const ref = ParamRef.of(col.envelope, {
          codec: { codecId: col.codecId },
        })
        row[col.name] = ref
    ...
    it('routes envelopes for each of the six cipherstash codec ids through bulk-encrypt', async () => {
      const { EncryptedDouble } = await import(
        '../src/execution/envelope-double'
      )
      const { EncryptedBigInt } = await import(
        '../src/execution/envelope-bigint'
      )
      const { EncryptedDate } = await import('../src/execution/envelope-date')
      const { EncryptedBoolean } = await import(
        '../src/execution/envelope-boolean'
      )
      ...
      const sdk = makeCounterSdk({
        encryptImpl: (args) =>
          args.values.map((_, i) => `ct:${args.routingKey.column}:${i}`),
      })
      const middleware = bulkEncryptMiddleware(sdk)
