...
`)
    ...
    if (!result.ok) return
    expect(
      asStorage(result.value.storage).tables['event']?.columns['occurredOn'],
    ).toMatchObject({
...
`)
    ...
    if (!result.ok) return
    expect(
      asStorage(result.value.storage).tables['feature']?.columns['enabled'],
    ).toMatchObject({
...
`)
    ...
    if (!result.ok) return
    expect(
      asStorage(result.value.storage).tables['audit']?.columns['payload'],
    ).toMatchObject({
