...
In a **Prisma Next** project there is deliberately no client file: encrypted columns are declared in the PSL contract. When the configured `client` path is missing, `encrypt` commands detect Prisma Next, read the emitted `contract.json` (searched at `src/prisma/`, `prisma/`, then the project root), and derive the schemas with the adapter's own `deriveStackSchemasV3` — so the CLI's schema view matches the application's. Both `@cipherstash/stack-prisma` and `@cipherstash/stack` are resolved from *your* project, not the CLI's dependency tree. Run `prisma-next contract emit` first; if the contract declares no `cipherstash.*()` column, the command says so rather than reporting a missing client file.
