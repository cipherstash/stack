...
describe('createSupabaseProvider getNextSteps', () => {
  ...
  it('uses npx when package manager is npm', () => {
    ...
    expect(steps[0]).toBe(
      'Install EQL: npx stash eql install --supabase (prompts for migration vs direct)',
    )
  })
  ...
  it('uses bunx when package manager is bun', () => {
    ...
    expect(steps[0]).toBe(
      'Install EQL: bunx stash eql install --supabase (prompts for migration vs direct)',
    )
  ...
  it('uses pnpm dlx when package manager is pnpm', () => {
    ...
    expect(steps[0]).toContain('pnpm dlx stash eql install --supabase')
  })

  it('uses yarn dlx when package manager is yarn', () => {
    const steps = provider.getNextSteps({}, 'yarn')
    expect(steps[0]).toBe(
      'Install EQL: yarn dlx stash eql install --supabase (prompts for migration vs direct)',
    )
