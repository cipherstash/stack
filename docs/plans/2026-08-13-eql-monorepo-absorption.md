...
- [ ] Delete `packages/eql/package.json` (the private `@cipherstash/eql-workspace` root). `packages/*` already matches `packages/eql`, so leaving the file in place silently enrols a second workspace root; with the file gone pnpm skips the directory, and the mise layer needs no `node_modules` to work. See the finding above for the script-by-script accounting.
- [ ] `pnpm-workspace.yaml`: add `packages/eql/packages/*`, so `@cipherstash/eql` is the only member the subtree contributes.
- [ ] Move the lockstep hook to the root, **in three parts — the script alone does nothing**:
...
- [ ] Fix the turbo **test** selection so it reaches the nested package: root `test` becomes `turbo test --filter './packages/**'`. Prefer the glob over a second explicit `--filter`, which goes stale; it is safe because the `platforms/*` packages define no scripts. `build` is already covered by `^build` traversal. **Mutation-check both:** break one of the nested package's unit tests and confirm root `pnpm test` fails; delete its `dist` and confirm root `pnpm build` recreates it.
...
- [ ] **Then** run `pnpm run code:fix` and commit the Biome 1.8.3 → 2.5.2 reflow **as its own commit**, so it never confounds a semantic diff. Verify by comparing token multisets rather than by eye, and confirm `mise run types:check` still passes afterwards — that gate is the thing this ordering protects.
...
- [ ] `packages/eql/package.json` is gone and `pnpm list --depth -1` shows no `@cipherstash/eql-workspace` member
- [ ] `sync-lockstep-versions.test.mjs` sits under `scripts/__tests__/` and root `pnpm test:scripts` actually runs it — mutation-checked by breaking an assertion
...
- [ ] No manifest names `eql-bindings` or `@cipherstash/eql` by registry version, except `packages/protect-ffi/integration-tests` under a named exemption with a written reason
...
1. ~~**A finished build against the path-dep `eql-bindings`.**~~ **Resolved 2026-08-13** — built clean in 3m 10s. See Phase 3. What remains unverified is the *credentialed* half: the `integration-tests` suite has not been run against the path dep, and that is what turns "no-op" from an argument about byte-identity into an observed result.
