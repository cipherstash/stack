...
#[sqlx::test(fixtures(path = "../fixtures", scripts("eql_v3_integer", "v3_ste_vec")))]
async fn uninstaller_preserves_application_tables_with_public_domain_columns(
    ...
    // These are generated by cipherstash-client for every test run. Preservation
    // must be proved with real ciphertext and index terms, not JSON shaped by
    // the test to happen to satisfy today's domain constraints.
    let scalar_payload: serde_json::Value =
        sqlx::query_scalar("SELECT payload FROM fixtures.eql_v3_integer ORDER BY id LIMIT 1")
            .fetch_one(&pool)
            .await?;
    let json_payload: serde_json::Value =
        sqlx::query_scalar("SELECT payload FROM fixtures.v3_ste_vec ORDER BY id LIMIT 1")
            .fetch_one(&pool)
            .await?;
    let entry_payload = json_payload
        .get("sv")
        .and_then(serde_json::Value::as_array)
        .and_then(|entries| entries.first())
        .cloned()
        .expect("generated SteVec fixture must contain at least one entry");
    ...
    .fetch_one(&pool)
    .await?;
    assert_eq!(values.0, scalar_payload);
    assert_eq!(values.1, json_payload);
    assert_eq!(values.2, entry_payload);
    ...
    assert_eq!(
        schema_count(&pool).await?,
        2,
        "reinstall must recreate both EQL-owned schemas without replacing application data"
    );
    let values_after_reinstall: (serde_json::Value, serde_json::Value, serde_json::Value) =
        sqlx::query_as(
            r#"
            SELECT
              scalar_value::jsonb,
        ...
        .await?;
    assert_eq!(
        values_after_reinstall, values,
        "install -> uninstall -> reinstall must preserve every stored byte"
    );

    Ok(())
}

#[sqlx::test(fixtures(path = "../fixtures", scripts("eql_v3_text", "v3_ste_vec")))]
async fn uninstall_and_reinstall_preserve_rows_for_every_public_eql_domain(
    pool: PgPool,
) -> Result<()> {
    let scalar_payload: serde_json::Value =
        sqlx::query_scalar("SELECT payload FROM fixtures.eql_v3_text ORDER BY id LIMIT 1")
            .fetch_one(&pool)
            .await?;
    let document_payload: serde_json::Value =
        sqlx::query_scalar("SELECT payload FROM fixtures.v3_ste_vec ORDER BY id LIMIT 1")
            .fetch_one(&pool)
            .await?;
    let entry_payload = document_payload
        .get("sv")
        .and_then(serde_json::Value::as_array)
        .and_then(|entries| entries.first())
        .cloned()
    ...
    let domains: Vec<String> = sqlx::query_scalar(
        ...
        WHERE n.nspname = 'public'
          AND t.typtype = 'd'
          AND t.typname LIKE 'eql_v3\_%' ESCAPE '\'
        ORDER BY t.typname
        "#,
    )
    .fetch_all(&pool)
    ...
    assert!(
        !domains.is_empty(),
        "installer must expose public data domains"
    );

    let mut expected = Vec::with_capacity(domains.len());
    for (index, domain) in domains.iter().enumerate() {
        // Names come from pg_type and are quoted before interpolation; values
        // remain bind parameters. One independent table per domain makes a
        // CASCADE-dropped column observable as missing data, not just a catalog
        // discrepancy.
        let quoted_domain = domain.replace('"', "\"\"");
        let table = format!("eql_v3_preserve_{index}");
        let payload = match domain.as_str() {
            "eql_v3_json_search" => &document_payload,
            "eql_v3_json_entry" => &entry_payload,
            _ => &scalar_payload,
        };
        sqlx::raw_sql(&format!(
            "CREATE TABLE public.{table} (id integer PRIMARY KEY, value public.\"{quoted_domain}\" NOT NULL)"
        ))
        .execute(&pool)
        .await?;
        sqlx::query(&format!(
            "INSERT INTO public.{table} VALUES (1, $1::jsonb::public.\"{quoted_domain}\")"
        ))
        .bind(payload)
        .execute(&pool)
        .await?;
        let identity: (i64, i16, i64, String) = sqlx::query_as(&format!(
            r#"
            SELECT c.oid::bigint, a.attnum, a.atttypid::bigint, a.attname::text
            FROM pg_catalog.pg_class c
            JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace
            JOIN pg_catalog.pg_attribute a ON a.attrelid = c.oid
            WHERE n.nspname = 'public' AND c.relname = '{table}' AND a.attname = 'value'
            "#
        ))
        .fetch_one(&pool)
        .await?;
        expected.push((table, payload.clone(), identity));
    }

    run_shipped_uninstaller(&pool).await?;
    for (table, payload, identity) in &expected {
        let actual: serde_json::Value = sqlx::query_scalar(&format!(
            "SELECT value::jsonb FROM public.{table} WHERE id = 1"
        ))
        .fetch_one(&pool)
        .await?;
        assert_eq!(&actual, payload, "uninstall changed data in {table}");
        let actual_identity: (i64, i16, i64, String) = sqlx::query_as(&format!(
            r#"
            SELECT c.oid::bigint, a.attnum, a.atttypid::bigint, a.attname::text
            FROM pg_catalog.pg_class c
            JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace
        ...
        assert_eq!(
            ...
            "uninstall changed column identity in {table}"
        );
    }
    ...
    for (table, payload, identity) in &expected {
        ...
        .fetch_one(&pool)
        .await?;
        assert_eq!(&actual, payload, "reinstall changed data in {table}");
        let actual_identity: (i64, i16, i64, String) = sqlx::query_as(&format!(
            r#"
            SELECT c.oid::bigint, a.attnum, a.atttypid::bigint, a.attname::text
            FROM pg_catalog.pg_class c
        ...
        assert_eq!(
            &actual_identity, identity,
            "reinstall changed column identity in {table}"
        );
    }
