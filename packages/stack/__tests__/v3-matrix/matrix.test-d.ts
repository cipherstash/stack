...
describe('eql_v3 type-driven matrix (types)', () => {
  ...
  it('excludes storage-only columns from the queryable set', () => {
    ...
    const ok: Queryable = V3_MATRIX['public.integer_eq'].builder('score')
  ...
  it('anchors the catalog key union to the real column source of truth', () => {
    ...
    expectTypeOf<'public.text_search'>().toExtend<EqlV3TypeName>()
    expectTypeOf<'public.boolean'>().toExtend<EqlV3TypeName>()
