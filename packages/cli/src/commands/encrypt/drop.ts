...
export async function dropCommand(options: DropCommandOptions) {
  ...
  try {
    ...
    // drop, after `cut-over`); v3 has no rename — the app switched to
    // `<col>_encrypted` by name, so the original `<col>` IS the plaintext
    // column, droppable straight after `backfilled`.
    const version = await detectColumnEqlVersion(
      client,
      options.table,
      `${options.column}_encrypted`,
    )
    ...
    if (isV3) {
      p.log.info(
        `${options.table}.${options.column}_encrypted is EQL v3 — the drop targets the original plaintext column "${options.column}" (v3 has no rename, so there is no "${options.column}_plaintext"). Make sure your application reads/writes ${options.column}_encrypted before applying this migration.`,
      )
    }
