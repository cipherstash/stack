...
| `@cipherstash/stack/wasm-inline` | The **edge** entry — Deno, Bun, Cloudflare Workers, Supabase Edge Functions. Its own `Encryption` factory plus its own copy of the v3 authoring surface, `EncryptionErrorTypes`, and the WASM build of protect-ffi inlined into the bundle. No native binding, so no bundler externalisation needed. **EQL v3 only** — `Encryption()` here rejects a v2 schema, and its operations return plain Results rather than thenable operations, so there is no `.audit()` chaining. **ESM-only, and its schema types do not interchange with the other entries'** — see the `stash-edge` skill for identity-bound encryption on this entry and the full native-vs-WASM difference table. |
