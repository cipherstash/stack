 ...
 * (it passes in isolation). Retry the live suites a couple of times so transient
 * throttling doesn't flake CI. Scoped here rather than in `vitest.config.ts` so
 * the pure/unit tests are never retried (a retry that hides a real pure-test bug
